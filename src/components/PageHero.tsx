...
export function PageHero({ eyebrow, title, subtitle, image, alt }: PageHeroProps) {
  return (
    <section className="relative h-[50vh] md:h-[60vh] min-h-[360px] md:min-h-[480px] flex items-end pb-10 md:pb-20 px-6 md:px-10 overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl reveal">
        <span className="eyebrow text-white/70 block mb-3 md:mb-6 tracking-[0.25em]">{eyebrow}</span>
        <h1
          className="font-serif text-[clamp(2.5rem,7vw,6.5rem)] leading-[0.95] md:leading-[0.9] text-white italic"
          style={{ textShadow: "0 2px 20px rgba(0,0,0,0.55)" }}
        >
          {title}
        </h1>
        {subtitle && (
          <p className="text-white/80 max-w-xl mt-4 md:mt-8 text-sm md:text-base leading-relaxed font-light">
            {subtitle}
          </p>
        )}
      </div>
