...
export const Route = createFileRoute("/dubai-activities")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Hand-curated Dubai experiences — desert safaris, sky-high dining, private yachts and architectural tours." },
      ...
      <main>
        ...
        <section className="py-32 px-6 md:px-10">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-20">
            {items.map((it, i) => (
              <article key={it.title} className={`group cursor-pointer ${i % 2 === 1 ? "md:mt-24" : ""}`}>
                <div className="overflow-hidden mb-6">
                  <img src={it.img} alt={it.alt} loading="lazy" className="w-full aspect-[4/5] object-cover group-hover:scale-105 transition-transform duration-700" />
                </div>
